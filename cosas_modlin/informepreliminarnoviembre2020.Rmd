...
En la expresión anterior n indica el número de iteraciones en cada cadena, W la media de las varianzas de las cadenas y B la varianza de las medias de las cadenas. Si el $\hat{R}$ se acerca a uno, eso significa que la variabilidad entre las cadenas es pequeña y por lo tanto la variabilidad de las simulaciones es explicada por la variabilidad dentro de las cadenas.

Por otro lado definimos el indicador:

$$n_{eff} = \frac{n.m}{1 + 2 \sum^{\infty}_{k = 1} \rho_k}$$
Donde $n$ es el número de iteraciones, $m$ es el número de cadenas y $\rho_k$ la autocorrelación $k-ésima$. Al estimar $\sum^{\infty}_{k = 1} \rho_k$ conseguimos $\hat{n}_{eff}$. Este indicador es una estimación de la cantidad de muestras independientes equivalentes a las del método MCMC. 
...
mod_bayes$stanfit
mod_bayes$coefficients
mcmc_trace(mod_bayes)
mcmc_trace(mod_bayes,pars="i440")

pred<-posterior_predict(mod_bayes,draws = 100)

a=pp_check(mod_bayes, plotfun = "stat", stat = "mean")

b=pp_check(mod_bayes, plotfun = "stat", stat = "median")
b
```
