...
#                       seed = 12345,
#                       iter = 4000) #Modelo viejo



mod_bayes <- stan_glm(log.salario ~ exper + tenure + female + reg.metro + norsur + comserv + profocc + expersq + educsq + i128 + i381 + i440,
                      data = datos2,
                      family = Gamma(link = "log"), #Probar sin el log a ver que pasa
                      seed = 12345,
                      iter = 4000)

pp_check(mod_bayes, nrep=100)


  #summary(mod_bayes)

#prior_summary(mod_bayes) #Información de las previas, informadas y ajustadas, para el intercepto, los coeficientes y sigma.

#summary.stanreg(mod_bayes)

parametros.clasicos <- summary(mod_clasico)$coefficients[,1]

summary.mod <- as.data.frame(mod_bayes$stanfit)
# parametros.bayesianos <- rep(0, 16)
# for (i in 1:16) {
#   parametros.bayesianos[i] <- mean(summary.mod[,i])
# }
#names(parametros.bayesianos) <- names(summary.mod)
